...
.component("ethComponent", {
    templateUrl:"api/api.py.php?_=account&a=eth",
    controller:["element", "request", "listen", "isEmpty", "$routeParams", "wait", "$route",function(element, request, listen, isEmpty, $routeParams, wait, $route){
       ...
       var qrcode_path  = '';
        ...
        request({
            method:"POST",
            url:"api/api.py.php?_=account&a=eth&t=s",
            formdata:false
        ...
        add_bitcoin_address = function(){
            request({
                method:"POST",
                url:"api/api.py.php?_=account&a=request_change&t=add&b=Ethereum",
                formdata:false
            }).then(function(response){
                response = JSON.parse(response)
        ...
        create_address = function(){
            ...
            request({
                method:"POST",
                url:"api/api.py.php?_=account&a=add_eth_address",
                formdata:true,
                data:data
            }).then(function(response){
                response = JSON.parse(response)
        ...
        ethereum_pan_view = function(id){
            data = new FormData()
            data.append("eth_id", id)
            request({
                method:"POST",
                url:"api/api.py.php?_=account&a=eth_pan_view",
                formdata:true,
                data:data
            }).then(function(response){
                response = JSON.parse(response)
        ...
        bitcoin_qrcode = function(amount){
            qcode = 'ethereum:'+bitcoin_address+'?value='+amount;
            var qr = new QRious({value:qcode, size:200});
            dataurl= qr.toDataURL();
            element("qrimg").src=dataurl
            // element("qrimg").style="width:200px; height:200px"
