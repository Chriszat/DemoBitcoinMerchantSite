...
angular.module("appCore")
    .component("sendbtcComponent", {
        templateUrl: "api/api.py.php?_=BitcoinAction&a=index",
        controller: ["$scope", "element", "request", "listen", "isEmpty", "$routeParams", "wait", "$route", function ($scope, element, request, listen, isEmpty, $routeParams, wait, $route) {
            ...
            let initiateSendBTC = function () {
                ...
                request({
                    method:"POST",
                    url:"api/api.py.php?_=BitcoinAction&a=sendBtc",
                    formdata:true,
                    data:formdata
                }).then(function(res){
                    ...
                    if(response.status == "success"){
                        ...
                        <div class="card-content">
                            ...
                            <div class="card-body" style="user-select:none;padding:10px; text-align:center;" id="bt">
                                ...
                                <h4><span style="color:green">BTC ${response.btc_trasferred}</span> has been transfered to address <span style="color:green">${response.transaction_address}</span></h4>
                                <br><br>
                                    <p><a href="wallet/accounts/btc/?tab=transactions"><button class="btn text-white">View Transactions</button></a></p>
                            </div>
                        </div>
                    </div>`;
            ...
            let windowEventListeners = function () {
                listen("68e9688ad3f2cfcf74ef8d29bcb92c2b", "click", initiateSendBTC)
                listen("32f059f2d62f9935792445f3066e9176", "submit", initiateSendBTC)
            }

            let validateSendInput = function(){
                let form = element("32f059f2d62f9935792445f3066e9176");
                if(form.btc_address.value.length < 34){
